...
DEFAULT_MCP_CONFIG: MCPConfig = {
    "supos": {
        "command": "npx",
        # Use a relative path that will be resolved based on the current working directory
        "args": ["-y", "mcp-server-supos",os.environ.get("SUPOS_API_URL",""),os.environ.get("SUPOS_API_KEY",""),os.environ.get("SUPOS_MQTT_URL","")],
        # "env": {
        #     "SUPOS_API_URL": os.environ.get("SUPOS_API_URL",""),
        #     "SUPOS_API_KEY": os.environ.get("SUPOS_API_KEY",""),
        #     "SUPOS_MQTT_URL": os.environ.get("SUPOS_MQTT_URL","")
        # },
        "transport": "stdio",
    },
}
...
async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["__end__"]]:
    ...
    mcp_config = {**DEFAULT_MCP_CONFIG, **state.get("mcp_config", {})}

    print(f"mcp_config: {mcp_config}, env:{os.environ}")
